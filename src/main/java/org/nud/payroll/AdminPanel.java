...
class AdminPanel extends JPanel {
    ...
    private final JComboBox<String> typeCombo =
            new JComboBox<>(new String[] {"Regular", "Probationary", "Contractual", "Part-time"});
    private final JTextField salaryField = PayrollSystem.styledField(20);
    private final JComboBox<String> cutOffCombo = new JComboBox<>(new String[] {"1 — 1st to 15th", "2 — 16th to 30th"});
    ...
    private JScrollPane buildAddPanel() {
        ...
        card.add(PayrollSystem.lbl(
                "Fill in all fields to register a new staff member.", PayrollSystem.F_BODY, PayrollSystem.C_MUTED));
        ...
        card.add(Box.createVerticalStrut(4));
        card.add(PayrollSystem.lbl(
                "The employee will use these to sign in.", PayrollSystem.F_SMALL, PayrollSystem.C_MUTED));
        card.add(Box.createVerticalStrut(16));
    ...
    private void saveEmployee() {
        ...
        if (!InputValidator.isValidEmployeeId(id)) {
            formMsg.setText("⚠  Invalid Employee ID (1–20 alphanumeric).");
            return;
        }
        if (!InputValidator.isValidName(name)) {
            formMsg.setText("⚠  Invalid Name (2–50 letters).");
            return;
        }
        ...
        try {
            ...
            formMsg.setText("⚠  Salary must be a number.");
            return;
        }
        if (!InputValidator.isValidSalary(salary)) {
            formMsg.setText("⚠  Salary must be ₱500 – ₱500,000.");
            return;
        }
        if (user.length() < 3) {
            formMsg.setText("⚠  Username must be at least 3 characters.");
            return;
        }
        if (pass.length() < 6) {
            formMsg.setText("⚠  Password must be at least 6 characters.");
            return;
        }
        ...
        double loan = 0;
        try {
            if (!slField.getText().trim().isEmpty())
                sl = Integer.parseInt(slField.getText().trim());
            if (!vlField.getText().trim().isEmpty())
                vl = Integer.parseInt(vlField.getText().trim());
            if (!elField.getText().trim().isEmpty())
                el = Integer.parseInt(elField.getText().trim());
            if (!initLoanField.getText().trim().isEmpty())
                loan = Double.parseDouble(initLoanField.getText().trim());
        } catch (NumberFormatException ex) {
            formMsg.setText("⚠  Leaves and Loans must be numbers.");
            return;
        }
    ...
    private JPanel buildViewPanel() {
        ...
        delBtn.addActionListener(e -> {
            ...
            int confirm = JOptionPane.showConfirmDialog(
                    this, "Delete employee " + empId + " and their account?", "Confirm", JOptionPane.YES_NO_OPTION);
